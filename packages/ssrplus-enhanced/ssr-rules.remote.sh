...
ipset_nft() {
	...
	router)
		...
		# dnsmasq 的 gfw_list.conf 在 router 模式下仍会尝试把国外域名写入 gfwlist。
		# 如果这里不预先创建，切换节点时 dnsmasq 会连续报错并导致分流解析抖动。
...
if [ -n "$server" ] && [ -n "$local_port" ]; then
	...
	if [ "$USE_NFT" = "1" ]; then
		...
		# Save previous server signature state
		SERVER_SIGNATURE_STATE_FILE="/tmp/.last_server_signature"
		CURRENT_SERVER_SIGNATURE="$(printf '%s|%s|%s|%s|%s|%s' "$server" "$local_port" "$SERVER" "$LOCAL_PORT" "$udp_server" "$udp_local_port")"
		if [ -f "$SERVER_SIGNATURE_STATE_FILE" ]; then
			LAST_SERVER_SIGNATURE=$(cat "$SERVER_SIGNATURE_STATE_FILE")
		else
		...
		if [ "$CURRENT_SERVER_SIGNATURE" != "$LAST_SERVER_SIGNATURE" ]; then
			loger 6 "Server signature changed: '$LAST_SERVER_SIGNATURE' -> '$CURRENT_SERVER_SIGNATURE'"
			SERVER_CHANGED=1
		else
			loger 6 "Server signature unchanged: '$CURRENT_SERVER_SIGNATURE'"
			SERVER_CHANGED=0
		fi
		...
		# 4. Active server / port signature has changed
		if [ "$TPROXY_HAS_VALUE" != "$LAST_HAS_VALUE" ] || \
		   [ "$PROXY_HAS_VALUE" != "$LAST_PROXY_HAS_VALUE" ] || \
		   [ "$ANY_IP_LIST_CHANGED" = "1" ] || \
		   [ "$SERVER_CHANGED" = "1" ]; then
			FORCE_RECREATE=1
			loger 6 "TPROXY, PROXY_PORTS, server signature or any IP list changed → force rebuild rules"
			rm -f "$NFTABLES_RULES_FILE" 2>/dev/null
		else
		...
		echo "$CURRENT_SERVER_SIGNATURE" > "$SERVER_SIGNATURE_STATE_FILE"
