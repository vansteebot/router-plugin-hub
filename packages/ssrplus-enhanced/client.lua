...
local function submitted_global_server()
	local current_uci = require "luci.model.uci".cursor()
	local explicit = trim_value(http.formvalue("ssrplus_apply_target"))
	if explicit ~= "" then
		return normalize_server_section(explicit, current_uci)
	end
...
local function submitted_or_current_global_option(option, default)
	local current_uci = require "luci.model.uci".cursor()
	local global = current_uci:get_first("shadowsocksr", "global")
	local candidates = {}
	if global and global ~= "" then
		table.insert(candidates, "cbid.shadowsocksr." .. global .. "." .. option)
	end
	table.insert(candidates, "cbid.shadowsocksr.@global[0]." .. option)
	for _, key in ipairs(candidates) do
		local value = http.formvalue(key)
		if value ~= nil then
	...
	return trim_value(current_uci:get_first("shadowsocksr", "global", option, default or ""))
end

local function validate_required_when(predicate, validator, required_message, invalid_message)
	return function(self, value, section)
		value = trim_value(value)
		if not predicate(section) then
			return value
		end
		if value == "" then
			return nil, required_message or translate("This field is required")
		end
		if validator and not validator(value) then
			return nil, invalid_message or translate("Invalid value")
		end
		return value
	end
end
...
cp.rmempty = true
cp.validate = validate_required_when(
	function()
		return submitted_or_current_global_option("dports", "1") == "3"
	end,
	function(value)
		return value:match("^[0-9,%-%s]+$") ~= nil
	end,
	translate("Custom ports cannot be empty when Custom Ports is selected."),
	translate("Invalid custom ports format.")
)
o.default = 1
...
o.rmempty = false
o.default = "0"
o.description = translate("定时探测当前节点；失败达到阈值后，只会在“服务器节点”页已勾选自动切换的候选节点里轮换。")

o = s:option(Flag, "monitor_enable", translate("启用守护进程监控"))
o.rmempty = true
o.default = "1"
o:depends("enable_switch", "1")
o.description = translate("主代理异常退出时，允许 watchdog 自动拉起。")

o = s:option(Value, "switch_time", translate("自动切换检测间隔（秒）"))
o.datatype = "uinteger"
o.default = "30"
o.rmempty = true
o:depends("enable_switch", "1")

o = s:option(Value, "switch_try_count", translate("连续失败阈值"))
o.datatype = "uinteger"
o.default = "0"
o.rmempty = true
o:depends("enable_switch", "1")
o.description = translate("达到这个次数后，才会从已勾选的候选节点里切换。填 0 表示只使用下面的滚动窗口阈值。")

o = s:option(Value, "switch_window_seconds", translate("滚动失败窗口（秒）"))
o.datatype = "uinteger"
o.default = "300"
o.rmempty = true
o:depends("enable_switch", "1")
o.description = translate("在这段时间内累计失败次数达到阈值时才切换。默认 300 秒，也就是 5 分钟。")

o = s:option(Value, "switch_window_failures", translate("窗口失败阈值"))
o.datatype = "uinteger"
o.default = "10"
o.rmempty = true
o:depends("enable_switch", "1")
o.description = translate("滚动窗口内失败达到这个次数才切换。填 0 表示关闭滚动窗口阈值。")

o = s:option(Value, "switch_cooldown", translate("自动切换冷却（秒）"))
o.datatype = "uinteger"
o.default = "300"
o.rmempty = true
o:depends("enable_switch", "1")
o.description = translate("每次自动切换完成后，至少等待这么久才允许下一次切换，避免反复抖动。")

o = s:option(Value, "switch_timeout", translate("单次探测超时（秒）"))
o.datatype = "uinteger"
o.default = "3"
o.rmempty = true
o:depends("enable_switch", "1")

o = s:option(Value, "switch_probe_host", translate("自动切换探测域名"))
o.default = "www.google.com"
o.rmempty = true
o:depends("enable_switch", "1")

o = s:option(Value, "switch_probe_port", translate("自动切换探测端口"))
o.datatype = "port"
o.default = "80"
o.rmempty = true
o:depends("enable_switch", "1")
...
if is_finded("chinadns-ng") then
	...
	o:value("223.5.5.5:53", translate("AliYun Public DNS (223.5.5.5) [推荐]"))
	...
	o:value("114.114.114.114:53", translate("Nanjing Xinfeng 114DNS (114.114.114.114) [不推荐,易被劫持]"))
	o:value("wan", translate("Use DNS from WAN [不推荐,依赖 ISP]"))
	o:value("wan_114", translate("Use DNS from WAN and 114DNS [不推荐]"))
	...
	o:value("", translate("Disable ChinaDNS-NG [境外站将不可访问,不推荐]"))
	...
	o.description = translate("Custom DNS Server format as IP:PORT. 默认 AliDNS (223.5.5.5); 不要选 ISP 下发 / 114DNS / Disable, 否则国内站会被解析到境外 CDN 或 GFW 投毒 IP.")
...
function m.on_after_apply(self)
	...
	if section and section ~= "" and section ~= "nil" then
		...
		global = current_uci:get_first("shadowsocksr", "global")
		if global then
			local stored = current_uci:get("shadowsocksr", global, "global_server")
			if stored ~= section then
				current_uci:set("shadowsocksr", global, "global_server", section)
				current_uci:commit("shadowsocksr")
			end
		end
	end
